...
**仅在 Phase 2.5 测试通过后触发。不直接上全自动 Cron。**

### Step 3.1: 半自动模式（默认 3 天）

```
测试通过，流水线效果确认 OK ✅

建议先跑 3 天半自动模式——每天自动采集选题+生成内容，
但发布前推送给你审核，你确认后才发。
...
cronjob action=create
  name: "{赛道名} 半自动试运行"
  schedule: "{用户选择的时间}"
  prompt: "使用 {赛道英文名} 流水线采集选题+生成内容，完成后发给用户审核，不要自动发布。附上内容预览和一键发布指令。"
  deliver: origin
```

### Step 3.2: 切换全自动

半自动跑满 3 天（或用户提前说"可以全自动了"），切换：

```
✅ 半自动试运行 3/3 天完成

3 天产出回顾：
- Day 1: {选题} → 你的反馈: {OK/调整了XX}
- Day 2: {选题} → 你的反馈: {OK/调整了XX}
- Day 3: {选题} → 你的反馈: {OK/调整了XX}
...
cronjob action=create
  name: "{赛道名} 全自动发布"
  schedule: "{用户选择的时间}"
  prompt: "使用 {赛道英文名} 流水线，采集选题→生成内容→自动发布，完成后推送发布报告。"
  deliver: origin
```
